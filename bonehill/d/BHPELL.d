...
IF ~GlobalLT("BHQuestAccept","GLOBAL",1) AreaCheck("AR9302")~ THEN BEGIN 0
  SAY @1
...
IF ~~ THEN BEGIN 11
  SAY @17
  IF ~!IfValidForPartyDialogue("%IMOEN_DV%")~ THEN DO ~EscapeArea()~ EXIT
  IF ~IfValidForPartyDialogue("%IMOEN_DV%")~ THEN DO ~EscapeArea()~ EXTERN ~%IMOEN_JOINED%~ PELLIMOEN
END
...
IF ~AreaCheck("%Candlekeep_Ch6%") Global("BHToldOfDeath","GLOBAL",1)~ THEN BEGIN 76
  SAY @158
...
IF ~AreaCheck("%Candlekeep_Ch6%") Global("BHToldOfDeath","GLOBAL",3)~ THEN BEGIN 81
  SAY @170
  IF ~~ THEN GOTO 82
END
